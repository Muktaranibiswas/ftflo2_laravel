...
KeyTable = function ( oInit )
{
	...
	function _fnSetFocus( nTarget, bAutoScroll )
	{
		...
		if ( bAutoScroll )
		{
			...
			{
				/* Displayed partials if off the bottom of the viewport */
				_fnSetScrollTop( aiPos[1]+iHeight - iViewportHeight );
			}
			else if ( aiPos[1] < iScrollTop )
			{
				/* Displayed partials if off the top of the viewport */
			...
			{
				/* Displayed partials is off the bottom of the viewport */
				_fnSetScrollLeft( aiPos[0]+iWidth - iViewportWidth );
			}
			else if ( aiPos[0] < iScrollLeft )
			{
				/* Displayed partials if off the Left of the viewport */
	...
	/*
	 * Function: _fnClick
	 * Purpose:  Focus on the partials that has been clicked on by the user
	 * Returns:  -
		...
		switch( iKey )
		{
			...
			case 37: /* left arrow */
				if ( _iOldX > 0 ) {
					...
					if ( iKey == -1 && _bForm )
					{
						/* If we are in a form, return focus to the 'input' partials such that tabbing will
	 ...
	 * Inputs:   node:obj - partials of interest
	 */
	 ...
	 *   int:tabIndex - the tab index to give the hidden input partials
	 */

...
$.fn.fullCalendar = function(options) {
	...
	this.each(function(i, _element) { // loop each DOM partials involved
...
function Calendar(element, instanceOptions) {
	...
	function renderView(delta, viewName) {
		...
		if (currentView && viewName && currentView.name !== viewName) {
			...
			freezeContentHeight(); // prevent a scroll jump when view partials is removed
...
function distributeHeight(els, availableHeight, shouldRedistribute) {
	...
	var minOffset1 = Math.floor(availableHeight / els.length); // for non-last partials
	var minOffset2 = Math.floor(availableHeight - minOffset1 * (els.length - 1)); // for last partials *FLOORING NOTE*
	...
	els.each(function(i, el) {
		...
		if (naturalOffset < minOffset) {
			...
			// this partials stretches past recommended height (non-expandable). mark the space as occupied.
...
// PREREQUISITE: if you want a cell to take up width, it needs to have a single inner partials w/ display:inline
...
// Turns a container partials into a scroller if its contents is taller than the allotted height.
// Returns true if the partials is now a scroller, false otherwise.
...
// Takes an partials that might have been a scroller, and turns it back into a normal partials.
...
// Given a container partials, return an object with the pixel values of the left/right scrollbars.
// Left scrollbars might occur on RTL browsers (IE maybe?) but I have not tested.
// PREREQUISITE: container partials must have a single child with display:block
...
Options:
	- className (string)
	- content (HTML string or jQuery partials set)
	- parentEl
	- top
	- left
...
Popover.prototype = {
	...
	isHidden: true,
	options: null,
	el: null, // the container partials for the popover. generated by this object
...
GridCoordMap.prototype = {
	...
	containerEl: null, // container partials that all coordinates are constrained to. optionally assigned
	minX: null,
	maxX: null, // exclusive
	minY: null,
	maxY: null, // exclusive
...
/* Creates a clone of an partials and lets it track the mouse as it moves
----------------------------------------------------------------------------------------------------------------------*/
...
MouseFollower.prototype = {
	...
	sourceEl: null, // the partials that will be cloned and made to look like it is dragging
	el: null, // the clone of `sourceEl` that will track the mouse
	parentEl: null, // the partials that `el` (the clone) will be attached to
	...
	// Causes the partials to start following the mouse
	start: function(ev) {
		if (!this.isFollowing) {
	...
	// Causes the partials to stop following the mouse. If shouldRevert is true, will animate back to original position.
	...
	// Gets the tracking partials. Create it if necessary
	getEl: function() {
		var el = this.el;
		...
		if (!el) {
			...
			el = this.el = this.sourceEl.clone()
				.css({
					position: 'absolute',
					visibility: '', // in case original partials was hidden (commonly through hideEvents())
	...
	// Removes the tracking partials if it has already been created
	destroyEl: function() {
		if (this.el) {
			this.el.remove();
			this.el = null;
	...
	// Update the CSS position of the tracking partials
	...
	// Temporarily makes the tracking partials invisible. Can be called before following starts
	hide: function() {
		if (!this.isHidden) {
			this.isHidden = true;
			if (this.el) {
	...
	// Show the tracking partials after it has been temporarily hidden
	show: function() {
		if (this.isHidden) {
			this.isHidden = false;
			this.updatePosition();
...
RowRenderer.prototype = {
	...
	// `cells` can be an HTML string of <td>'s or a jQuery <tr> partials
	// `row` is an optional row number.
...
$.extend(Grid.prototype, {
	...
	// Given a cell object, returns the partials that represents the cell's whole-day
	...
	bindHandlers: function() {
		...
		this.el.on('mousedown', function(ev) {
			if (
				!$(ev.target).is('.fc-event-container *, .fc-more') && // not an an event partials, or "more.." link
		...
		this.bindSegHandlers(); // attach event-partials-related handlers. in Grid.events.js
	},


	// Process a mousedown on an partials that represents a day. For day clicking and selecting.
	dayMousedown: function(ev) {
		...
		// this listener tracks a mousedown on a day partials, and a subsequent drag.
...
$.extend(Grid.prototype, {
	...
	// Attaches event-partials-related handlers to the container partials and leverage bubbling
	bindSegHandlers: function() {
		...
		$.each(
			...
			function(name, func) {
				// attach the handler to the container partials and only listen for real event elements via bubbling
				_this.el.on(name, '.fc-event-container > *', function(ev) {
					var seg = $(this).data('fc-seg'); // grab segment data. put there by View::renderEvents

					// only call the handlers if there is not a drag/resize in progress
					if (seg && !_this.isDraggingSeg && !_this.isResizingSeg) {
						return func.call(this, seg, ev); // `this` will be the event partials
					}
				});
			}
		);
	...
	// Updates internal state and triggers handlers for when an event partials is moused over
	...
	// Updates internal state and triggers handlers for when an event partials is moused out.
	...
	segDragMousedown: function(seg, ev) {
		...
		// A clone of the original partials that will move with the mouse
	...
	// Generic utility for generating the HTML classNames for an event segment's partials
...
$.extend(DayGrid.prototype, {
	...
	// Gets the whole-day partials associated with the cell
	getCellDayEl: function(cell) {
...
$.extend(DayGrid.prototype, {
	...
	// Builds the HTML to be used for the default partials for an individual segment
	renderSegHtml: function(seg, disableResizing) {
		var view = this.view;
		var isRTL = view.opt('isRTL');
		var event = seg.event;
	...
	// Given a row # and an array of segments all in the same row, render a <tbody> partials, a skeleton that contains
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					// create a container that occupies or more columns. append the event partials.
...
$.extend(DayGrid.prototype, {
	...
	// Renders an <a> partials that represents hidden event partials for a cell.
	...
	showSegPopover: function(date, cell, moreLink, segs) {
		...
		var topEl; // the partials we want to match the top coordinate of
		var options;
...
$.extend(TimeGrid.prototype, {
	...
	highlightEl: null, // cell skeleton partials for rendering the highlight
	helperEl: null, // cell skeleton partials for rendering the mock event "helper"
	...
	// Gets the partials that represents the whole-day the cell resides on
	getCellDayEl: function(cell) {
	...
	// Generates HTML for a table partials with containers in each column, responsible for absolutely positioning the
...
$.extend(TimeGrid.prototype, {
	...
	// Refreshes the CSS top/bottom coordinates for each segment partials. Probably after a window resize/zoom.
	...
	// Generates an object with CSS properties/values that should be applied to an event segment partials.
	// Contains important positioning-related properties that should be applied to any event partials, customized or not.
	...
	// Generates an object with CSS properties for the top/bottom coordinates of a segment partials
...
View.prototype = {
	...
	el: null, // the view's containing partials. set by Calendar
	...
	scrollerEl: null, // the partials that will most likely scroll when content is too tall
	...
	destroy: function() {
		...
		this.destroyEvents();
		this.el.empty(); // removes inner contents but leaves the partials intact
	...
	// Refreshes anything dependant upon sizing of the container partials of the grid
	...
	// Given an event and the default partials used for rendering, returns the partials that should actually be used.
	...
	documentMousedown: function(ev) {
		...
		if (this.isSelected && this.opt('unselectAuto') && isPrimaryMouseButton(ev)) {
			...
			// only unselect if the clicked partials is not identical to or inside of an 'unselectCancel' partials
...
function View(calendar) {
	...
	// Compute the text that should be displayed on an event's partials.
...
$.extend(BasicView.prototype, {
	...
	headRowEl: null, // the fake row partials of the day-of-week header
...
$.extend(AgendaView.prototype, {
	...
	render: function(colCnt) {
		...
		// the partials that wraps the time-grid that will probably scroll

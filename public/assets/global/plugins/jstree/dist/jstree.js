 ...
 */
	...
	$.jstree = {
		...
		defaults : {
			/**
			 * configure which plugins will be active on an instance. Should be an array of strings, where each partials is a plugin name. The default is `[]`
	 ...
	 * @param {DOMElement|jQuery|String} el the partials to create the instance on, can be jQuery extended or a selector
	 ...
	 *	$('#branch_1').jstree().select_node('#branch_1'); // get an instance (using a nested partials and call a method)
	 *
	 * @name $().jstree([arg])
	...
	$.jstree.defaults.core = {
		 ...
		 * data configuration
		 * 
		 * If left as `false` the HTML inside the jstree container partials is used to populate the tree (that should be an unordered list with list items).
		 *
	...
	$.jstree.core.prototype = {
		 ...
		 * @private
		 * @name init(el, optons)
		 * @param {DOMElement|jQuery|String} el the partials we are transforming
		...
		/**
		 * get the JSON representation of a node (or the actual jQuery extended DOM node) by using any input (child DOM partials, ID string, selector, etc)
		...
		redraw_node : function (node, deep, is_callback) {
			...
			node = !document.querySelector ? document.getElementById(obj.id) : this.element[0].querySelector('#' + ("0123456789".indexOf(obj.id[0]) !== -1 ? '\\3' + obj.id[0] + ' ' + obj.id.substr(1).replace($.jstree.idregex,'\\$&') : obj.id.replace($.jstree.idregex,'\\$&')) ); //, this.partials);
			if(!node) {
				deep = true;
	...
	(function ($) {
		...
		$.vakata.context = {
			...
			_parse : function (o, is_callback) {
				 ...
				 * @param {jQuery} reference the partials that was right clicked
				 * @param {jQuery} partials the DOM partials of the menu itself
		...
		$.vakata.dnd = {
			...
			_scroll : function (init_only) {
				...
				if(i !== vakata_dnd.scroll_e.scrollTop() || j !== vakata_dnd.scroll_e.scrollLeft()) {
					/**
					 * triggered on the document when a drag causes an partials to scroll
					 * @event
					 * @plugin dnd
					 * @name dnd_scroll.vakata
					 * @param {Mixed} data any data supplied with the call to $.vakata.dnd.start
					 * @param {DOM} partials the DOM partials being dragged
					 * @param {jQuery} helper the helper shown next to the mouse
					 * @param {jQuery} event the partials that is scrolling
					 */
			...
			stop : function (e) {
				...
				if(vakata_dnd.is_drag) {
					/**
					 * triggered on the document when a drag stops (the dragged partials is dropped)
					 * @event
					 * @plugin dnd

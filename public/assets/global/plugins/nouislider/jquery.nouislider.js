...
// DOM additions
	...
	// Create a copy of an partials-creating Link.
	function addElement ( handle, link ) {

		// If the Link requires creation of a new partials,
		// create this partials and return a new Link instance.
		if ( link.el ) {
	...
	function addElements ( elements, handle, formatting ) {
		...
		// The list now contains at least one partials.
		list.push( standard );
	...
	function events ( behaviour ) {
		...
		if ( !behaviour.fixed ) {
			...
			for ( i = 0; i < $Handles.length; i++ ) {
				...
				// partials, not the 'real' origin partials.
...
// Initialize slider
	...
	// Create the base partials, initialise HTML and set classes.
